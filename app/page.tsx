...
type NoteColor = "yellow" | "pink" | "blue" | "green" | "lavender";
...
const NOTE_COLORS_DARK: Record<NoteColor, string> = {
  yellow: "#E8CC3A",
  pink: "#FF7098",
  blue: "#5AAEFF",
  green: "#4DD98A",
        ...
        <div className="flex gap-0.5 opacity-0 group-hover:opacity-30 transition-opacity cursor-grab">
          {[0, 1, 2].map((r) => (
            <div key={r} className="flex flex-col gap-[3px]">
              {[0, 1].map((c) => <div key={c} className="w-[3px] h-[3px] rounded-full bg-stone-700" />)}
            </div>
          ))}
        </div>
